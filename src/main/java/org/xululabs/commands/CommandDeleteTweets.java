	...
	public TtafResponse execute() throws Exception {
		...
		try {
			...
			// Function to read files in directory getting all files in directory
				...
				List<String> encriptedNamesAndGid = new ArrayList<>();
				if (file1.exists()) {
					
					// loading file which contain index Encryption for all users
					encriptedNamesAndGid = UtilFunctions.loadFile(this.getFilePath()+"/indexEncription");
			}
				else {
				...
				cal.add(Calendar.DATE, -3);
				 String keyword=  format.format(cal.getTime()).toString();
				  ...
				  for (String indexEncriptionAndGid : encriptedNamesAndGid) {
				     boolean success = false;
				     String indexEncription = indexEncriptionAndGid.split("=")[0];
				     String nodeId = indexEncriptionAndGid.split("=")[1];
				     if (date.isEmpty()) {
							date = keyword+"-"+nodeId;
						}
