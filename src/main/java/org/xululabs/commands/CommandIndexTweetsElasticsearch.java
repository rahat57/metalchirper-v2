...
public class CommandIndexTweetsElasticsearch extends BaseCommand {
	...
	public TtafResponse execute() throws Exception {
		...
		for (int f = 0; f <fileNames.size(); f++) {
		 ...
		 int gId = Integer.parseInt(fileParts[1]);
		String indexEncription = mysql.getIndexEncryptionByGid(this.getDbName(),this.getDbUser(),this.getDbPassword(),uId_gid);
		...
		Date date = new Date();
		String type = formatter.format(date)+"-"+ gId;
			try {				
